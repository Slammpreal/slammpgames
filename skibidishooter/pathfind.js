'use strict';
{
  ...
  self.JobHandlers["PFFindPath"] = function(params) {
    ...
    const directMovementMode =
      params["directMovementMode"];
    const pathfinder = GetPathfinder(mapKey);
    ...
    if (pathfinder.IsInPathGroup())
      if (pathGroup) {
        if (pathfinder.GetPathGroupId() !== pathGroup["id"]) {
          pathfinder.EndGroup();
          pathfinder.StartGroup(pathGroup["id"], pathGroup["cost"], pathGroup["cellSpread"])
        }
      } else pathfinder.EndGroup();
    else if (pathGroup) pathfinder.StartGroup(pathGroup["id"], pathGroup["cost"], pathGroup["cellSpread"]);
    pathfinder.SetMoveCost(moveCost);
    pathfinder.SetDiagonalsEnabled(diagonalsEnabled);
    const result = pathfinder.FindPath(cellX, cellY, destCellX, destCellY, directMovementMode);
    return {
      result
    }
  };
  ...
  class Pathfinder {
    constructor() {
      ...
      this._cells = null;
      this._openList =
        new self.RedBlackSet(Node.Sort);
    ...
    At(x, y) {
      if (x < 0 || y < 0 || x >= this._hcells || y >= this._vcells) return PF_OBSTACLE;
      let ret = this._cells[x][y];
      if (this._groupCostCells !== null) ret = Math.min(ret + this._groupCostCells[x][y], PF_OBSTACLE);
      return ret
    }
    ...
    FindPath(startX, startY, endX, endY, directMovementMode) {
      ...
      if (directMovementMode !== 0 && this.IsBoxAllClear(startX, startY, endX, endY)) return [{
        x: endX,
        y: endY
      }];
      return this._AStarFindPath(startX, startY, directMovementMode)
    }
    _AStarFindPath(startX,
      startY, directMovementMode) {
      ...
      while (!openList.IsEmpty()) {
        ...
        if (diagonals && !obsLeft && !obsTop && this.At(x - 1, y - 1) !== PF_OBSTACLE) this._AddCellToOpenList(x - 1, y - 1, diagonalCost);
        if (!obsTop) this._AddCellToOpenList(x, y - 1, adjacentCost);
        if (diagonals &&
          !obsTop && !obsRight && this.At(x + 1, y - 1) !== PF_OBSTACLE) this._AddCellToOpenList(x + 1, y - 1, diagonalCost);
        if (!obsRight) this._AddCellToOpenList(x + 1, y, adjacentCost);
        if (diagonals && !obsRight && !obsBottom && this.At(x + 1, y + 1) !== PF_OBSTACLE) this._AddCellToOpenList(x + 1, y + 1, diagonalCost);
        if (!obsBottom) this._AddCellToOpenList(x, y + 1, adjacentCost);
        if (diagonals && !obsBottom && !obsLeft && this.At(x - 1, y + 1) !== PF_OBSTACLE) this._AddCellToOpenList(x - 1, y + 1, diagonalCost)
      }
      this._ClearIntermediateData();
      return null
    }
    _AddCellToOpenList(x,
      y, g) {
    ...
    _EstimateH(x, y) {
      ...
      return this._diagonalsEnabled ? manhattanDist + (this._diagonalCost - 2 * this._adjacentCost) * Math.min(dx, dy) : manhattanDist
    }
    ...
    _AddPathGroupCost(pathList) {
      ...
      const spreadOffset = Math.floor(cellSpread /
        2);
    ...
    _FilterRedundantNodes(pathList) {
      ...
      for (let i = 1, len = pathList.length; i < len; ++i) {
        ...
        if (i === len - 1) retList.push(curNode);
          ...
          const bothXNearZero = Math.abs(nextDx) < NZ_EPSILON && Math.abs(curDx) < NZ_EPSILON && Math.sign(nextDy) ===
            Math.sign(curDy);
          const bothYNearZero = Math.abs(nextDy) < NZ_EPSILON && Math.abs(curDy) < NZ_EPSILON && Math.sign(nextDx) === Math.sign(curDx);
          if (!bothXNearZero && !bothYNearZero && Math.abs(nextDx / curDx - nextDy / curDy) > R_EPSILON) {
      ...
      while (i < len) {
        ...
        if (i >= len - 2) ++i;
          ...
          while (j < len) {
            ...
            if (!this.IsBoxAllClear(curNode.x, curNode.y, tryNode.x, tryNode.y)) break
          }
        }
      }
      return retList
